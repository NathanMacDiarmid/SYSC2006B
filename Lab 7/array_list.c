...
{
	assert(new_capacity != list->size);
	assert(list != NULL);

	intlist_t *new;

	new->elems = malloc(new_capacity * sizeof(int));

	for (int i = 0; i < new_capacity; i++) {

		new->elems[i] = list->elems[i];

	}

	list->capacity = new_capacity;
	list->elems = new->elems;
...
void intlist_delete(intlist_t *list, int index)
{
	...
	assert(!(list->size >= 0 && list->size <= (list->size - 1)));
	assert(list != NULL);
